...
- **Sample guards on venue-frequency gates**: Do not let sample-inadequate venue-seasons (for example neutral-site or outdoor games with one or two observations) drive event-frequency residual max-z pass/fail decisions or the league mean/std baseline used to compute those z-scores. Report those rows as `insufficient_evidence` diagnostics, but build frequency z-score baselines and the primary event-frequency acceptance gate only from sample-adequate regular-season training-attempt venue-seasons. This keeps one-off event environments visible without letting them hide or create stable scorekeeper-regime signals.
- **No future evidence in regime-aware gates**: Any validation gate that can waive a blocking residual based on historical/regime context must compute that context with information available at the evaluated season only. Do not classify a venue-season as `persistent_bias`, supported, or otherwise non-blocking because a future season matches it. Centered rolling windows may be displayed as exploratory diagnostics, but pass/fail decisions must use prior-only or current-and-prior evidence and tests must cover the first spike in a repeated sequence.
- **Notebook cell IDs**: When adding or rewriting `.ipynb` cells programmatically, include stable `id` fields on every new cell. Missing IDs trigger nbformat `MissingIDFieldWarning` during execution and will become hard errors in future nbformat versions.
