...
- **Sample guards on venue-frequency gates**: Do not let sample-inadequate venue-seasons (for example neutral-site or outdoor games with one or two observations) drive event-frequency residual max-z pass/fail decisions. Report those rows as `insufficient_evidence` diagnostics, but compute the primary event-frequency acceptance gate only on sample-adequate regular-season training-attempt venue-seasons. This keeps one-off event environments visible without mistaking them for stable scorekeeper regimes.
