...
- Updated `index.md` - refreshed Last updated summary.
**Notes:** The venue-correction scorecard now separates distance/location residuals from event-frequency residuals. The live DB-backed run still passes held-out log-loss and home-ice guardrails, but fails corrected-distance residuals (`max |z| = 4.067`, worst `20092010:Madison Square Garden`) and sample-adequate regular-season training-attempt frequency residuals (`max |z| = 3.453`, worst `20152016:Prudential Center`). One-off neutral/outdoor venues remain visible as `insufficient_evidence` diagnostics instead of driving the primary frequency gate.
