...
| Phase 2.5 — rigor foundation (new, gates Phase 3) | **In progress** | 2.5.1 and 2.5.2 are implemented in `src/` with tests; 2.5.4 now has an initial correction table (`venue_bias_corrections`), shrinkage-based distance adjustment parameters wired into `finalize_season_diagnostics`, and a hold-out acceptance evaluator (`evaluate_venue_correction_holdout`) with over-correction guardrail tests in `tests/test_validation.py`; 2.5.5 now has a recorded decision and an enforced loader guard (`load_training_shot_events`) excluding pre-2009 seasons. 2.5.3 remains open. |
...
- ⏸ Venue correction layer has initial implementation (`venue_bias_corrections` table + shrinkage distance adjustments) but still requires held-out acceptance validation and over-correction guardrail checks under Phase 2.5.4.
